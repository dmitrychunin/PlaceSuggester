...
import data.{distinctPlacesSeq, manualJoinedPersonWithPlacesSchema, personsSeq, placesSeq, topNearestPlacesForEachPersonSchema}
import org.apache.spark.sql.{DataFrame, Dataset, SparkSession}
import org.scalatest.{BeforeAndAfter, FunSuite}

//todo rename and move to package
class HelloWorldTest extends FunSuite with SharedSparkContext with DatasetSuiteBase with BeforeAndAfter {
...
//  todo use DatasetGenerator?
  ...
  test("filterDuplicatedPlaces should filter places with not distinct latitude and longitude combination") {
    val placesWithDistinctLatitudeAndLongitude: Dataset[Place] = HelloWorld.filterDuplicatedPlaces(places)
  ...
  test("joinPersonsAndPlacesWithDistanceColumn should join two dataset and add distance column") {
    val joinPersonsAndPlacesWithDistance = HelloWorld.joinPersonsAndPlacesWithDistanceColumn(persons, distinctPlaces)
  ...
  test("selectTopNNearestPlacesForEachPerson should keep only top N nearest to person places") {
    val actualTopNearestDF = HelloWorld.selectTopNNearestPlacesForEachPerson(joinedPersonWithPlaces, 1)(spark)
